...
function loadConfig() {
    return {
        ...
        thinkingMode: process.env.GLM_THINKING_MODE || "interleaved"
    };
}
// GLM API 호출
async function callGlmApi(config, systemPrompt, userMessage, options) {
...
// 병렬 GLM 호출 (앙상블)
async function callGlmParallel(config, userMessage, roles) {
    const results = {};
...
server.tool("glm_chat", "GLM-4.7에 단일 질의를 수행합니다", {
...
server.tool("glm_ensemble", "4개 역할(분석/검토/최적화/보안)로 병렬 분석을 수행합니다", {
    ...
    const selectedRoles = roles || ["analyst", "reviewer", "optimizer", "security"];
    try {
...
server.tool("glm_config", "현재 GLM 설정을 확인합니다", {}, async () => {
    const config = loadConfig();
